...
function computePrevWeek(): { from: string; to: string } {
...
function formatDateRange(from: string, to: string) {
  ...
  return `${f.toLocaleDateString("ru-RU", opts)} — ${t.toLocaleDateString("ru-RU", { ...opts, year: "numeric" })}`;
}
...
export default async function PulseWeekPage() {
  const period = computePrevWeek();
  const supabase = await createServerSupabase();
  const kpis = await getKpis(supabase, { from: period.from, to: period.to }, "all", "day");
  ...
  return (
    <div className="space-y-8">
      ...
      <PlaceholderSection icon={Package} title="Склад и логистика" items={["Остатки ТЗ в рублях (конец недели)", "Сумма товаров в пути", "Сумма списаний центрального склада"]} />
    </div>
  );
}
